...
void FlightPhyics::operator()(float dt, float controls[3], float throttle){
    ...
    btVector3 thrust = flightDir * max_thrust * thottle;
...
btVector3 FlightPhyics::getForward(){
    ...
    return trans.inverse()(btVector(1,0,0)); //May need to change this to another unit vector;
}

btVector3 FlightPhyics::getUp(){
    btTransform trans;
    mRigidBody->getMotionState()->getWorldTransform(trans);
    return trans.inverse()(btVector(0,1,0)); //May need to change this to another unit vector;
}